...
| **F2** | Cohort A2: `/plans` (`Map<PlanTypeId, (FeatureKey \| string)[]>` derived signal with warn-and-keep per D5) + `/Taxes` (Authorize-aware fetch; ETag added; Dexie row keyed by `/Taxes?countryCode=…`). `BusinessService` settings flow updated to consume the new cached signal. | F1 | Medium |
| **F4** | Cohort C: `/macro-categories` adoption (new signal `macroCategories()`); `/plan-types` adoption (new signal); `/access-methods` adoption (new signal); `/access-reasons` migrated from Promise to signal (FR-012). `resolveMacro(businessTypeId)` helper introduced. `macroOfBusinessType(id)` deleted from `config.enum.ts`. `TenantContextService.currentBusinessType()?.hasKitchen` rewired to use `resolveMacro()`. | F1 | High |
